...
function esbuildTransformSync(rawSource, filename, url, format) {
  const {
    ...
    // The next valid target for Node.js 14 (which is the current supported
    // version in AWS Lambda) would be "es2020", but it doesn't include top
    // level await (and AWS Lambda does!), and neither does "es2021". The next
    // target is "es2022", so that's what we will use for now. Ideally, we
    // would stick to "node14" but can't until either Pulumi's function
    // serialization improves, or until there's a way to disable the esbuild
    // spread helpers.
    //
    // The following issues are worth tracking:
    // - https://github.com/evanw/esbuild/issues/2060
    target: "es2022",
